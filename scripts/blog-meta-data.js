(function(){
  ...
  // This function contains the core logic to build and inject the JSON-LD.
  function processMetaData(wordCount) {
    // This log is now inside the function to show when it actually runs.
    console.log(`[Meta Data] Processing meta data with word count: ${wordCount || 'N/A'}.`);
  ...
  // This handles the race condition where word-count.js might finish before this script runs.

  // Check if the word count is already available (if word-count.js ran first).
  if (window.__revrebel_wc && window.__revrebel_wc.ready) {
    console.log('[Meta Data] Word count was ready on load. Processing immediately.');
    processMetaData(window.__revrebel_wc.count);
  } else {
    // Otherwise, set up the listener and wait for the event.
    console.log('[Meta Data] Script loaded. Waiting for "wordCountReady" event...');
    document.addEventListener('wordCountReady', (event) => processMetaData(event.detail.count));
  }
})();
