...
<!-- START PageNav Show/Hide -->

<script
  defer
  src="https://cdn.jsdelivr.net/gh/REVREBEL/rebel-style@main/scripts/page-nav-visibility.js"
  type="text/javascript"
  referrerpolicy="no-referrer"
  crossorigin="anonymous">
</script>

<!-- END PageNav Show/Hide  -->
...
3) Tweak live from console: pageNavMarkSet({ anchor: 0.5, startOffset: 0, endOffset: 0 })
   ...
   - force always on/off: pageNavForceAlways(true/false)
   - enable logs: window.PAGE_NAV_DEBUG = true
-->
*/
 ...
 *    - `pageNavMarkSet({anchor?, startOffset?, endOffset?})`
 *    - `pageNavForceAlways(boolean)`
 *
 * Debug:
 * - Set `window.PAGE_NAV_DEBUG = true` to enable console logging.
 *
 * Markup requirements:
 * - Wrapper element:  `aside.page-nav_wrapper`
 * - Start sentinel:   any element with `[data-pagenav-start]`
 * - End sentinel:     any element with `[data-pagenav-end]`
 */


(function(){
  if (window.__pageNavMarkersInit) return; 
  window.__pageNavMarkersInit = true;

  /** @type {string} */
  var WRAPPER_SEL = 'aside.page-nav_wrapper ';
  /** @type {string} */
  var START_SEL   = '[data-pagenav-start]';
  /** @type {string} */
  var END_SEL     = '[data-pagenav-end]';

  // ---------------- Tunables (mutable at runtime via pageNavMarkSet) ----------------
  ...
  /**
   * Conditional logger (enabled by `window.PAGE_NAV_DEBUG = true`).
   * @param {...any} args
   * @returns {void}
   */
  function log(){ 
    if(!window.PAGE_NAV_DEBUG) return; 
    try{ 
      console.log.apply(console, ['[pageNav markers]'].concat([].slice.call(arguments))); 
    }catch(_){} 
  }
  ...
  ready(function(){
    ...
    window.pageNavMarkSet = function(opts){
      opts = opts || {}; 
      var changed=false;
     ...
     * @returns {void}
     */
    window.pageNavForceAlways = function(on){
      on = !!on; 
      wrapper.classList.toggle('is-visible', on);
      if (on){ wrapper.style.opacity='1'; wrapper.style.pointerEvents='auto'; }
      else   { wrapper.style.opacity='';  wrapper.style.pointerEvents=''; }
