...
(function(){
  if (window.__revShareFailSafeInit) return; window.__revShareFailSafeInit = true;

  // ======= Quick toggles you can set in console =======
  // window.REV_SHARE_ALWAYS_ON = true  -> pins visible for debugging
  // window.REV_SHARE_DEBUG = true      -> console logs

  var WRAP_SEL = 'aside.share-icons-wrapper';
  var SEC_SEL  = '[data-section="blog-detail"], .blog-detail';

  // IO tuning
  var THRESHOLD   = 0.06;           // show when ~6% of section is visible
  var ROOT_MARGIN = '0px 0px -120px 0px'; // keep visible closer to bottom of section

  function log(){ if(!window.REV_SHARE_DEBUG) return; try{ console.log.apply(console, ['[revShare]'].concat([].slice.call(arguments))); }catch(_){} }
  function ready(fn){ if(document.readyState !== 'loading') fn(); else document.addEventListener('DOMContentLoaded', fn, {once:true}); }

  ready(function(){
    var w = document.querySelector(WRAP_SEL);
    var sec = document.querySelector(SEC_SEL);
    if(!w){ log('wrapper not found'); return; }
    if(!sec){ log('section not found'); /* still allow manual pin */ }

    // 1) Immediate assert-visible pass (proves CSS works)
    //    We keep it visible until the first computed decision runs.
    var firstDecisionMade = false;
    function assertVisible(){
      w.classList.add('is-visible');
      // Defensive inline styles (beat most CSS)
      w.style.opacity = '1';
      w.style.pointerEvents = 'auto';
    }
    assertVisible();

    // 2) MutationObserver: if some other script strips the class, we re-apply and log.
    var mo;
    try{
