...
<!-- START Solution Progess Bar Show/Hide -->

<script
  defer  
  src="https://cdn.jsdelivr.net/gh/REVREBEL/rebel-style@main/scripts/progress-bar-visibility.js"
  type="text/javascript"
  referrerpolicy="no-referrer"
  crossorigin="anonymous">
</script>

<!-- END Solution Progess Bar  Show/Hide  -->
...
1) In Webflow, add two tiny Divs anywhere in the page:
   - one with Custom Attribute:  name = data-progress-bar-start, value = (blank)
   - one with Custom Attribute:  name = data-progress-bar-end,   value = (blank)
...
3) Tweak live from console: progressBarMarkSet({ anchor: 0.5, startOffset: 0, endOffset: 0 })
   ...
   - force always on/off: progressBarForceAlways(true/false)
   - enable logs: window.PROGRESS_BAR_DEBUG = true
-->
*/

/**
 * Component: Progress Bar Visibility via Scroll Markers
 *
 * Toggles `is-visible` on the wrapper (`aside.solutions-progress-bar`) when the
 * viewport "anchor line" (a fraction of the viewport height) lies between two
 * marker elements: `[data-progress-bar-start]` and `[data-progress-bar-end]`.
 *
 * Behavior:
 ...
 *    - `progressBarMarkSet({anchor?, startOffset?, endOffset?})`
 *    - `progressBarForceAlways(boolean)`
 *
 * Debug:
 * - Set `window.PROGRESS_BAR_DEBUG = true` to enable console logging.
 *
 * Markup requirements:
 * - Wrapper element:  `aside.solutions-progress-bar`
 * - Start sentinel:   any element with `[data-progress-bar-start]`
 * - End sentinel:     any element with `[data-progress-bar-end]`
 */


(function(){
  if (window.__progressBarMarkersInit) return; 
  window.__progressBarMarkersInit = true;

  /** @type {string} */
  var WRAPPER_SEL = 'aside.solutions-progress-bar ';
  /** @type {string} */
  var START_SEL   = '[data-progress-bar-start]';
  /** @type {string} */
  var END_SEL     = '[data-progress-bar-end]';

  // ---------------- Tunables (mutable at runtime via progressBarMarkSet) ----------------
  ...
  /**
   * Conditional logger (enabled by `window.PROGRESS_BAR_DEBUG = true`).
   * @param {...any} args
   * @returns {void}
   */
  function log(){ 
    if(!window.PROGRESS_BAR_DEBUG) return; 
    try{ 
      console.log.apply(console, ['[progressBar markers]'].concat([].slice.call(arguments))); 
    }catch(_){} 
  }
  ...
  ready(function(){
    ...
    if (!startEl || !endEl){ log('markers missing (need both data-progress-bar-start and data-progress-bar-end)'); return; }
    ...
    window.progressBarMarkSet = function(opts){
      opts = opts || {}; 
      var changed=false;
     ...
     * @returns {void}
     */
    window.progressBarForceAlways = function(on){
      on = !!on; 
      wrapper.classList.toggle('is-visible', on);
      if (on){ wrapper.style.opacity='1'; wrapper.style.pointerEvents='auto'; }
      else   { wrapper.style.opacity='';  wrapper.style.pointerEvents=''; }
